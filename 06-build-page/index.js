...
(async () => {
  ...
  const filesCss = await fsPromises.readdir(path.join(__dirname, 'styles'), {
    withFileTypes: true,
  });
  ...
  const templateData = await fsPromises.readFile(
  ...
  const filesHtml = await fsPromises.readdir(
  ...
  for (const item of matches) {
    ...
    for (const file of filesHtml) {
      if (file.name === name) {
        ...
        templateStringForReplace = templateStringForReplace.replace(
          item,
          data.toString(),
        );
      }
  ...
  for (const file of files) {
    if (file.isDirectory()) {
      ...
      const deepFiles = await fsPromises.readdir(`${file.path}/${file.name}`, {
        withFileTypes: true,
      });
      for (const deepFile of deepFiles) {
        await fsPromises.copyFile(
          `${deepFile.path}/${deepFile.name}`,
          `${path.join(__dirname, 'project-dist/assets')}/${file.name}/${
            deepFile.name
          }`,
        );
